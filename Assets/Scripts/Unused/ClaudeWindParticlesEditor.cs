			...
			if (foldOutConfigs)
			{
				using (new EditorGUI.IndentLevelScope())
				{
					...
					*/
					...
					for (int i = 0; i < obj.particles.Count; i++)
					{
						var particle = obj.particles[i];
						using (new EditorGUILayout.HorizontalScope(GUILayout.ExpandWidth(true)))
						{
							var boxColour = particle.color;
							boxStyle.normal.background.SetPixel(0, 0, boxColour);
							boxStyle.normal.background.Apply();
							GUILayout.Box("", boxStyle, GUILayout.Width(EditorGUIUtility.singleLineHeight), GUILayout.ExpandWidth(false));

							obj.particles[i].originPosition = EditorGUILayout.Vector3Field("Origin", particle.originPosition);
